...
data_collection = load('Expected Aggregate Investment LQRE.py.data')
...
plot_artists = ax.plot(x[keep], y[keep], 'o', color='#0433ff', markersize=3, alpha=0.8, zorder=1, label='2 Players', mec='none')
...
y = layer_data['col3']
keep = ~np.isnan(x) & ~np.isnan(y)
plot_artists = ax.plot(x[keep], y[keep], 'o', color='#aa7942', markersize=3, alpha=0.8, zorder=2, label='3 Players', mec='none')
layer_handles.extend(plot_artists)
...
y = layer_data['col3']
keep = ~np.isnan(x) & ~np.isnan(y)
plot_artists = ax.plot(x[keep], y[keep], 'o', color='#00f900', markersize=3, alpha=0.8, zorder=4, label='5 Players', mec='none')
layer_handles.extend(plot_artists)
...
y = layer_data['col3']
keep = ~np.isnan(x) & ~np.isnan(y)
plot_artists = ax.plot(x[keep], y[keep], 'o', color='#ff40ff', markersize=3, alpha=0.8, zorder=5, label='6 Players', mec='none')
layer_handles.extend(plot_artists)
...
ax.set_ylim(bottom=5.0, top=19.0)
...
ax.set_ylabel('Expected Aggregate Investment', weight='normal', size=10)
